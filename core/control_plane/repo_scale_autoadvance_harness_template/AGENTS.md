# Repo-Scale Auto-Advance Harness
...
- Do not stop after an intermediate report if the patch/run gate is already
  open.
- Do not patch the stale snapshot, the legacy distractor, or the unrelated
  dirty files.
- Do not replace execution with a status-only summary when the patch/run gate
  is already open.
