...
run_root="${3:-/tmp/worktree_remap_regression_${timestamp}}"

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
materialize="${script_dir}/materialize_worktree_remap_harness.sh"
...
for i in $(seq 1 "$count"); do
  ...
  if ! rg -q 'return a \+ b' "${case_dir}/math_bug.py"; then
    echo "case_${i}: FAIL active worktree file not patched"
    continue
  fi

  if ! rg -q 'return a - b' "${case_dir}/snapshots/clean/math_bug.py"; then
    echo "case_${i}: FAIL clean snapshot was modified"
    continue
  fi
